//! Fuzzes bounded durable-operation decoding for deterministic preservation.
#![no_main]

use libfuzzer_sys::fuzz_target;
use recourse::client::{DecodeError, DecodeLimit, DecodeLimits, ReceivedOperationDiagnostic};

fuzz_target!(|body: &[u8]| {
    let first = ReceivedOperationDiagnostic::from_slice(body, DecodeLimits::default());
    let second = ReceivedOperationDiagnostic::from_slice(body, DecodeLimits::default());
    match (first, second) {
        (Ok(first), Ok(second)) => {
